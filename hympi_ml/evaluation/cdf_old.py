...
scalers_base = "/data/nature_run/work/src"
...
def load_data_cpl(label_set, day, sensor):
    ...
    if sensor == "atms":
        ...
        s_factors = np.load(f"{scalers_base}/minimac_scaling_factors_mh.npz")
        mins = s_factors["mins"]
        maxs = s_factors["maxs"]
        ...
        s_factors = np.load(f"{scalers_base}/minimac_scaling_factors_cpl_hsel.npz")
        mins = s_factors["mins"]
        maxs = s_factors["maxs"]
        rad = np.nan_to_num(rad)
        rad = (rad - mins) / (maxs - mins)
    ...
    s_factors = np.load(f"{scalers_base}/minimac_scaling_factors_cpl_cpl.npz")
    mins = s_factors["mins"]
    maxs = s_factors["maxs"]
    cpl = (cpl - mins) / (maxs - mins)

    nrun_sca = data["scalar"]
    nrun_tab = data["table"]

    scale_spress = np.load(f"{scalers_base}/spress_scalar.npz")
    mins = scale_spress["mins"]
    maxs = scale_spress["maxs"]
    spress_unscaled = nrun_sca[:, 3]
...
def load_data(label_set, day, sensor):
    ...
    if sensor == "atms":
        ...
        s_factors = np.load(f"{scalers_base}/minimac_scaling_factors_hsel.npz")
        mins = s_factors["mins"]
        maxs = s_factors["maxs"]
        rad = np.nan_to_num(rad)
        rad = (rad - mins) / (maxs - mins)
