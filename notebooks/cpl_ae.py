...
mlflow_logging.start_run(
    ...
    experiment_name="CPL Autoencoder",
    log_datasets=False,
)

(train, validation, test) = fd.get_split_datasets(
    feature_names=[DKey.CPL],
    target_name=DKey.CPL,
    train_days=[
        "20060315",
        "20060515",
        "20060615",
...
encoder = LayerNormalization()(cpl_input)
encoder = Dense(128, act.swish)(encoder)

encoder = Dense(latent_size, act.swish)(encoder)

decoder = encoder
decoder = Dense(128, act.swish)(decoder)
decoder = Dense(train.target.data_shape[0])(decoder)
model = Model(cpl_input, decoder)

encoder = Model(cpl_input, encoder)
...
var = plots.plot_pred_truth_var(train_pred, train_truth, context=f"{steps} Train Batches", show=True)
mlflow.log_figure(var, var.get_suptitle() + ".png")
