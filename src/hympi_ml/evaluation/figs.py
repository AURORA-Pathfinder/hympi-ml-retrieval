...
# class FigureData:
#     """
#     The common set of data used for generating a specific figure.
#     """
...
#     @abstractmethod
#     def get_figure(self, data: FigureData) -> Figure:
#         """
...
#         mean_error = data.pred - data.truth
...
#         value_label = data.units

#         if value_label is None:
#             value_label = data.data_name
...
#         fig.suptitle(f"{data.data_name} {self.title} ({data.context})")
...
#         profiles = self.get_profiles(data.pred, data.truth)
#         value_label = data.units
...
#     def get_figure(self, data: FigureData) -> Figure:
#         super_fig = super().get_figure(data)
...
#             if data.pred.ndim == 1 and isinstance(eval_figure, EvalProfile):
#                 continue

#             if data.pred.ndim > 1 and not isinstance(eval_figure, EvalProfile):
#                 continue

#             fig = eval_figure.get_figure(data)

#             if log:
#                 mlflow_log.log_figure(
#                     fig,
#                     artifact_file=f"{data.context}_figures/{fig.get_suptitle()}.png",
#                 )
...
#                 data_name=name,
#                 context=context,
#                 units=dataset.targets[name].units,
#             )
#             for name in preds.keys()
#         ]

#         figs += plot_eval_figures(datas, eval_figures, log)
